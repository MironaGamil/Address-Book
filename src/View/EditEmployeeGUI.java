...
public class EditEmployeeGUI {
	...
	public void update(){
		...
		if (valid){
			execute = control.MySQLAccess.update_employee(id, name, company, home, homephone, business, busphone, phone);
			if (execute){
				update_fax(id);
			}	
		}
	...
	public void update_fax(String id){
		...
		for(int i=0; i< parts.length; i++){
			if(!parts[i].equals("")){
				execute = control.MySQLAccess.insert_to_faxnumbers(Integer.parseInt(id), parts[i]);
				if (!execute){
