		...
		try {
			...
			for (int temp = 0; temp < nList.getLength(); temp++) {
				...
				if (nNode.getNodeType() == Node.ELEMENT_NODE) {
					...
					int id = control.MySQLAccess.insert_to_employee(name, cName, hAddr,
							hNumber, bAddr, bNumber, cNumber);

					if (id != -1)
						get_fax_numbers(faxNumbers, id);
	...
	public static void get_fax_numbers(NodeList faxNumbers, int id) {
		...
		for (int i = 0; i < faxNumbers.getLength(); i++) {
			if (faxNumbers.item(i).getNodeType() == Node.ELEMENT_NODE) {
				...
				control.MySQLAccess.insert_to_faxnumbers(id, faxNumber);
			}
		}
	}
		...
		try {
			...
			for (int temp = 0; temp < nList.getLength(); temp++) {
				...
				if (nNode.getNodeType() == Node.ELEMENT_NODE) {
					...
					control.MySQLAccess.insert_to_agents(name, address, phone);
